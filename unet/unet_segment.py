# -*- coding: utf-8 -*-
"""
Created on Tue Nov 12 13:44:48 2019
...
IMG_CHANNELS = 3
TRAIN_PATH = 'P:/opencv/unet/input/stage1_train/'
TEST_PATH = 'P:/opencv/unet/input/stage1_test/'

warnings.filterwarnings('ignore', category=UserWarning, module='skimage')
seed = 42
#Preparing the data 
# Get train and test IDs
#w = os.walk('P:/opencv/unet/input/stage1_train')
...
for n, id_ in tqdm(enumerate(train_ids), total=len(train_ids)):
    ...
    for mask_file in next(os.walk(path + '/masks/'))[2]:
        ...
        #mask_=mask_.reshape(len(inn_train),1)
        #print(mask_file)
...
checkpointer = ModelCheckpoint('nmn_cell_segment_unet-3.h5', verbose=1, save_best_only=True)
...
model = load_model('nmn_cell_segment_unet-3.h5', custom_objects={'mean_iou': mean_iou})
...
imshow(X_train[ix])
plt.show()
#cv.imwrite('trainingsample2.png',X_train[ix])
imshow(np.squeeze(Y_train[ix]))
plt.show()
#imsave('annotated2.png',img_as_uint(np.squeeze(Y_train[ix])))
imshow(np.squeeze(preds_train_t[ix]))
plt.show()
#imsave('predicted2.png',img_as_uint(np.squeeze(preds_train_t[ix])))
#cv.imwrite('predicted2.png',preds_train_t[ix])
